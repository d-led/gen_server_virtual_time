...
# Script to generate all CAF (C++ Actor Framework) example projects
# Usage: mix run scripts/generate_caf_examples.exs

defmodule CAFExampleGenerator do
  @moduledoc """
  Generates all CAF example projects for testing and validation.
  """

  def run do
    IO.puts("""
    ╔═══════════════════════════════════════════════════════════╗
    ║  Generating CAF Example Projects                          ║
    ╚═══════════════════════════════════════════════════════════╝
    """)

    examples = [
      {:pubsub, &create_pubsub_simulation/0, "PubSubActors"},
      {:pipeline, &create_pipeline_simulation/0, "PipelineActors"},
      {:burst, &create_burst_simulation/0, "BurstActors"},
      {:loadbalanced, &create_loadbalanced_simulation/0, "LoadBalancedActors"}
    ]
  ...
  defp generate_example(name, simulation, project_name) do
    ...
    try do
      ...
      output_dir = "examples/caf_#{name}"
      :ok = ActorSimulation.CAFGenerator.write_to_directory(files, output_dir)

      file_count = length(files)
      IO.puts("✅ Generated #{file_count} files in #{output_dir}/")

      # List key files
      cpp_count = Enum.count(files, fn {fname, _} -> String.ends_with?(fname, ".cpp") end)
      hpp_count = Enum.count(files, fn {fname, _} -> String.ends_with?(fname, ".hpp") end)
      IO.puts("   - #{cpp_count} C++ source files")
      IO.puts("   - #{hpp_count} C++ header files")
      IO.puts("   - CMakeLists.txt, conanfile.txt, CI pipeline")

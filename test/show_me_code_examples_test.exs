defmodule ShowMeCodeExamplesTest do
  ...
  describe "Show Me The Code examples" do
    test "GenServerVirtualTime example works" do
      ...
      VirtualClock.advance(clock, 100_000)  # 100s virtual, fast real
      
      Process.sleep(50)  # Let messages process
      
      count = GenServer.call(server, :get_count)
      assert count >= 99  # Should have ~100 work items
    ...
    test "Actor DSL with aliases works" do
      ...
      # Should have sent ~1000 messages (100/sec * 10sec)
      assert stats.actors[:producer].sent_count >= 900
      assert stats.actors[:consumer].received_count >= 900
